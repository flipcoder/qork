...
class Player(Mesh):
    def __init__(self, *args, **kwargs):
        ...
        self.scale(1.5)
        self.states({"life": "alive", "stance": "stand", "direction": "up"})
...
class App(Core):
    def __init__(self, **kwargs):
        ...
        self.keybinds = ["S", "F", "E", "D", "SPACE", "A", "J", "L"]
    ...
    def update(self, dt):
        ...
        # move
        v = vec3(
            (-1 if self.btns[LEFT] else 0) + (1 if self.btns[RIGHT] else 0),
            (-1 if self.btns[CROUCH] else 0) + (1 if self.btns[JUMP] else 0),
            (-1 if self.btns[UP] else 0) + (1 if self.btns[DOWN] else 0),
        )
